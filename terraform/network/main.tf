...
resource "oci_core_network_security_group" "permit_kube_api" {
  compartment_id = var.compartment_id
  vcn_id         = oci_core_vcn.cluster_network.id
  display_name   = "Permit SSH"
}

resource "oci_core_network_security_group_security_rule" "permit_kube_api" {
  network_security_group_id = oci_core_network_security_group.permit_kube_api.id
  protocol                  = "6" // TCP
  source                    = var.managemnet_network
  source_type               = "CIDR_BLOCK"
  tcp_options {
